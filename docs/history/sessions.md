...
For design documentation see `docs/README.md`.
...
| Trading-day perf run | **Required** to close the bug entry: compare p99 at 2^21 / 2^22 / 2^23 against the **first run, 2026-08-08** — 96 ms / 733 ms / >1 s ([trading_day_load.md](../operations/trading_day_load.md#a-hash-rehash-stalls-the-hot-path-for-over-a-second)). Previously cited here as run 5's, which is wrong: run 5 is the cancels run and its largest spike was ~60 ms. The first run is the right baseline because its 2^23 failure was pure latency — 16 GB free on a 31 GB machine — whereas runs 8/9/10 were OOM-killed at that step and measured exhaustion instead |
