...
## 2.8 DSL Schema Subsystem
The DSL subsystem defines the schema language used to describe all binary
message payloads in the system. It provides:

- A single source of truth for message structure
- Strongly typed fields, enums, nested messages, lists, and optional fields
- UTF‑8 string support and variable‑length payloads
- Deterministic, allocator‑friendly binary encoding rules
- A Python-based code generator that emits self‑contained C++17 headers

The DSL ensures that message formats are explicit, versioned, and free from
layout drift. All encode/decode logic for payloads is generated from DSL files.

See: [DSL Grammar Specification](dsl_grammar.md)
