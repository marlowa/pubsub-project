...
[WAL and High Availability](../availability/wal_and_ha.md) for the full commit and replication semantics.
...
- [WAL and High Availability](../availability/wal_and_ha.md) — WAL format, two-tier commit, replication protocol, failover
- [Sequencer Application](sequencer_app.md) — operational detail: port allocation, TOML config, startup order, implemented slices
- [Socket Comms](../framework/socket_comms.md) — inline PDU handler mechanism (`InlinePduHandler`, `InstallInlinePduHandler`)
