...
*(`start_fix_seq_system.py` did this job until 2026-08-27. It was deleted as superseded — it named four configuration files that had not existed since the HA `_primary`/`_secondary` rename, so it failed at its first component. See BUG-0016.)*
...
[cpu_pinning_anti_affinity.md](cpu_pinning_anti_affinity.md) -- but it is
...
[cpu_pinning.md](cpu_pinning.md)); then consider `isolcpus` with `nohz_full`
...
[cpu_pinning_anti_affinity.md](cpu_pinning_anti_affinity.md). Summary here
