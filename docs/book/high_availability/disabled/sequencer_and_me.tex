...
--- is the case this subsection exists for. It is in the log, so the venue took
it. The engine does not hold it, so nothing will answer it. And with both components restarted,
nothing knows it is owed: the sequencer's record that it was waiting did not
survive, and the engine holds nothing to compare against.
...
\begin{requirement}{R-0017}{Recovery does not depend on the order the components return in}
  The venue shall reach the same state whichever of the two starts first.
  \rationale{A launcher restarts what it starts and coordinates with nothing, so
  the order is not the venue's to choose. A component that cannot yet be told
