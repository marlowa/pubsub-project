...
\begin{requirement}{R-0116}{The venue recovers what it held before deciding what to do with it}
  ...
  \uncovered{no scenario restarts the engine and inspects what it holds before trading resumes}
\end{requirement}
...
\begin{requirement}{R-0117}{Orders open across a long absence are cancelled, reported, and trading halts}
  ...
  without halting.
  \rationale{The market moved while the venue could not match, and the member
  could not cancel because there was nothing to cancel against. An order left
  resting at a price struck before that gap is a hazard to the member that placed
  it, and the hazard has nothing to do with whether the order was recoverable ---
  a correctly recovered stale order is exactly as dangerous as a correctly
  recovered fresh one.
  ...
  \reqref{R-0023}.}
  \uncovered{no scenario keeps the engine down long enough for the period to elapse}
\end{requirement}
...
\begin{requirement}{R-0118}{How long an absence may be before orders are cancelled is a stated figure}
  ...
  the last order it matched to trading resuming.
...
\begin{requirement}{R-0102}{A checkpoint that cannot be trusted is not used}
  ...
  one.}
  \uncovered{no scenario damages a checkpoint}
\end{requirement}
