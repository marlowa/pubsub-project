...
class Pybind11Generator:
    ...
    def _emit_functions_binding(self, msg: MessageDecl, w):
        ...
        w('            if (!ns::decode(view, buf.data, buf.size, consumed, decode_arena, arena_bytes_needed))')
        w('                throw std::runtime_error("decode failed");')
        w('            return view;')
        w('        }')
        w('        if (py::isinstance<py::bytes>(obj)) {')
        w('            std::string tmp = obj.cast<std::string>();')
        w('            if (!ns::decode(view, reinterpret_cast<const std::uint8_t*>(tmp.data()), tmp.size(), consumed, decode_arena, arena_bytes_needed))')
        w('                throw std::runtime_error("decode failed");')
        w('            return view;')
        w('        }')
