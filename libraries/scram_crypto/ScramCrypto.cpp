...
std::vector<uint8_t> hmac_sha256(const uint8_t* key, size_t key_size, const uint8_t* data, size_t data_size) {
    unsigned char result[32];
    unsigned int length = 32;
    if (HMAC(EVP_sha256(), key, static_cast<int>(key_size), data, data_size, result, &length) == nullptr) {
...
std::vector<uint8_t> pbkdf2_sha256(std::string_view password, const uint8_t* salt, size_t salt_size, int32_t iterations) {
    std::vector<uint8_t> result(32);
    if (!PKCS5_PBKDF2_HMAC(password.data(), static_cast<int>(password.size()), salt, static_cast<int>(salt_size), iterations, EVP_sha256(), 32,
                           result.data())) {
...
ScramCredential make_scram_credential(std::string_view password, const uint8_t* salt, size_t salt_size, int32_t iterations) {
    ...
    const std::vector<uint8_t> client_key =
        hmac_sha256(salted_password.data(), salted_password.size(), reinterpret_cast<const uint8_t*>(client_key_label.data()), client_key_label.size());

    const std::vector<uint8_t> stored_key = sha256(client_key.data(), client_key.size());

    const std::vector<uint8_t> server_key =
        hmac_sha256(salted_password.data(), salted_password.size(), reinterpret_cast<const uint8_t*>(server_key_label.data()), server_key_label.size());

    return ScramCredential{std::vector<uint8_t>(salt, salt + salt_size), iterations, stored_key, server_key};
}

std::vector<uint8_t> compute_auth_message(std::string_view comp_id, const std::vector<uint8_t>& client_nonce, const std::vector<uint8_t>& server_nonce,
                                          const uint8_t* salt, size_t salt_size, int32_t iterations) {
