    ...
    for (;;) {
        ...
        if (!header_validated_) {
            ...
            PUBSUB_LOG(logger_, FwLogLevel::Trace,
                       ...
                       header_buffer_[14], header_buffer_[15], header_buffer_[16], header_buffer_[17], header_buffer_[18], header_buffer_[19],
            ...
            PUBSUB_LOG(logger_, FwLogLevel::Trace, "TRACE PduParser::receive: connection_id={} allocate returned slab_id={} chunk={}",
...
void PduParser::dispatch_pdu(SlabHandle slab_id, void* payload_chunk) {
    ...
    if (logger_.quill_logger()->should_log_statement(quill::LogLevel::Debug)) {
        ...
        if (logger_.log_level() <= FwLogLevel{FwLogLevel::Trace}) {
