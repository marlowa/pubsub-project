...
class TcpAcceptorImpl {
  public:
    ...
    TcpAcceptorImpl(const InetAddress& local_address, std::unique_ptr<TcpSocket> listening_socket)
        : local_address_(local_address), listening_socket_(std::move(listening_socket)) {}

    TcpAcceptorImpl(const TcpAcceptorImpl&) = delete;
    TcpAcceptorImpl& operator=(const TcpAcceptorImpl&) = delete;

    TcpAcceptorImpl(TcpAcceptorImpl&& other) : local_address_(std::move(other.local_address_)), listening_socket_(std::move(other.listening_socket_)) {}
...
TcpAcceptor::TcpAcceptor(std::unique_ptr<TcpAcceptorImpl> p_impl) : p_impl_(std::move(p_impl)) {}
...
std::tuple<std::unique_ptr<TcpAcceptor>, std::string> TcpAcceptor::create(const InetAddress& local_address, int backlog) {
    // 1. Create socket
    auto [socket_ptr, create_error] = TcpSocket::create(local_address.get_sockaddr()->sa_family);
    if (!socket_ptr) {
        return {nullptr, fmt::format("Failed to create listening TcpSocket: {}", create_error)};
    }
    ...
    if (!bind_success) {
        return {nullptr,
                fmt::format("Failed to bind listening socket to {}:{}: {}", local_address.get_ip_address_string(), local_address.get_port(), bind_error)};
    }

    // 3. Listen
    auto [listen_success, listen_error] = socket_ptr->listen(backlog);
    if (!listen_success) {
        return {nullptr, fmt::format("Failed to set listening socket to listen mode: {}", listen_error)};
    }
...
std::tuple<std::unique_ptr<TcpSocket>, std::unique_ptr<InetAddress>, std::string> TcpAcceptor::accept_connection() {
    if (!p_impl_) {
