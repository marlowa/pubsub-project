...
class FileLock {
  public:
    explicit FileLock(const std::string& path) : file_path_(path) {
        ...
        if (fd_ < 0) {
            throw PubSubItcException("Failed to open lock file '" + file_path_ + "': " + StringUtils::get_errno_string());
        }
    ...
    FileLock(FileLock&& other) : fd_(other.fd_), file_path_(std::move(other.file_path_)) {
        other.fd_ = -1;
    }
