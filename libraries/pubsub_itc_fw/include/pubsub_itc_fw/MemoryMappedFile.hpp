...
/**
    ...
    This class encapsulates the platform-specific details of creating, mapping, and unmapping a file to memory, providing a simple, fast, and durable storage
   solution for an ApplicationThread's state.
*/
...
*/
template <typename StateType> class MemoryMappedFile {
  public:
    /*
        The destructor ensures that the memory-mapped file is unmapped and the file handle is closed, guaranteeing that all data is flushed to disk and
    ...
    */
    ...
    /**
        ...
        This constructor attempts to open an existing file or create a new one at the specified path. It then maps the file into memory, allowing direct access
       to the state data.
