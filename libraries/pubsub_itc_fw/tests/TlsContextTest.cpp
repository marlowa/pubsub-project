...
void add_basic_constraints(X509* x509, bool is_ca) {
    X509_EXTENSION* ext = X509V3_EXT_conf_nid(nullptr, nullptr, NID_basic_constraints, is_ca ? "critical,CA:TRUE" : "critical,CA:FALSE");
    if (ext) {
...
X509* make_signed(EVP_PKEY* subject_key, const char* cn, X509* ca_cert, EVP_PKEY* ca_key) {
    X509* x = X509_new();
    if (!x)
        return nullptr;
    X509_set_version(x, 2);
...
class TlsContextTest : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        X509* server_cert = (server_key && ca_cert) ? make_signed(server_key, "Test Server", ca_cert, ca_key) : nullptr;
        ...
        if (ok) {
            ...
            ok = write_pem_cert(ca_cert_path_, ca_cert) && write_pem_cert(server_cert_path_, server_cert) && write_pem_key(server_key_path_, server_key) &&
