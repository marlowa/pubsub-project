...
class PduProtocolHandlerConnectorThread : public ApplicationThread {
  public:
    PduProtocolHandlerConnectorThread(ConstructorToken token, QuillLogger& logger, Reactor& reactor)
        : ApplicationThread(token, logger, reactor, "PduProtocolHandlerConnectorThread", ThreadID{1}, make_queue_config(),
                            make_allocator_config("PduProtocolHandlerConnectorPool"), make_connector_thread_config()) {}
...
class PduProtocolHandlerListenerThread : public ApplicationThread {
  public:
    PduProtocolHandlerListenerThread(ConstructorToken token, QuillLogger& logger, Reactor& reactor)
        : ApplicationThread(token, logger, reactor, "PduProtocolHandlerListenerThread", ThreadID{2}, make_queue_config(),
                            make_allocator_config("PduProtocolHandlerListenerPool"), make_listener_thread_config()) {}
...
class LargeResponseListenerThread : public ApplicationThread {
  public:
    LargeResponseListenerThread(ConstructorToken token, QuillLogger& logger, Reactor& reactor)
        : ApplicationThread(token, logger, reactor, "LargeResponseListenerThread", ThreadID{2}, make_queue_config(),
                            make_allocator_config("LargeResponseListenerPool"), make_large_response_listener_thread_config()) {}
...
class SmallQueryConnectorThread : public ApplicationThread {
  public:
    SmallQueryConnectorThread(ConstructorToken token, QuillLogger& logger, Reactor& reactor)
        : ApplicationThread(token, logger, reactor, "SmallQueryConnectorThread", ThreadID{1}, make_queue_config(),
                            make_allocator_config("SmallQueryConnectorPool"), make_small_query_connector_thread_config()) {}
...
TEST_F(PduProtocolHandlerIntegrationTest, LargeQueryNameForcesPartialsend) {
    ...
    auto listener_thread = ApplicationThread::create<PduProtocolHandlerListenerThread>(logger_->logger, *listener_reactor);
    ...
    auto connector_thread = ApplicationThread::create<PduProtocolHandlerConnectorThread>(logger_->logger, *connector_reactor);
    ...
    EXPECT_TRUE(wait_for([&]() { return connector_thread->query_sent.load(std::memory_order_acquire); })) << "Connector: large DataQuery not sent";

    EXPECT_TRUE(wait_for([&]() { return listener_thread->query_received.load(std::memory_order_acquire); }, 15000))
        << "Listener: large DataQuery not received within timeout";
    ...
    EXPECT_TRUE(listener_thread->received_query_name_all_x) << "Received query_name contained unexpected characters";
...
TEST_F(PduProtocolHandlerIntegrationTest, LargeResponseForcesPartialSend) {
    ...
    auto listener_thread = ApplicationThread::create<LargeResponseListenerThread>(logger_->logger, *listener_reactor);
    listener_reactor->register_thread(listener_thread);
    ...
    EXPECT_TRUE(wait_for([&]() { return listener_thread->response_sent.load(std::memory_order_acquire); })) << "Listener: large DataResponse not sent";

    EXPECT_TRUE(wait_for([&]() { return connector_thread->response_received.load(std::memory_order_acquire); }, 15000))
        << "Connector: large DataResponse not received within timeout";
...
TEST_F(PduProtocolHandlerIntegrationTest, LargeQueryNameForcesOutboundWriteReady) {
    ...
    auto connector_reactor = std::make_unique<Reactor>(make_small_sndbuf_connector_reactor_config(), connector_registry, logger_->logger);
...
class DisconnectingListenerThread : public ApplicationThread {
  public:
    DisconnectingListenerThread(ConstructorToken token, QuillLogger& logger, Reactor& reactor)
        : ApplicationThread(token, logger, reactor, "DisconnectingListenerThread", ThreadID{2}, make_queue_config(),
                            make_allocator_config("DisconnectingListenerPool"), ApplicationThreadConfiguration{}) {}
...
TEST_F(PduProtocolHandlerIntegrationTest, ListenerClosesConnection) {
    ...
    auto listener_thread = ApplicationThread::create<DisconnectingListenerThread>(logger_->logger, *listener_reactor);
    listener_reactor->register_thread(listener_thread);
    ...
    EXPECT_TRUE(wait_for([&]() { return listener_thread->connection_lost.load(std::memory_order_acquire); })) << "Listener: ConnectionLost not received";
...
class DoubleSendConnectorThread : public ApplicationThread {
  public:
    DoubleSendConnectorThread(ConstructorToken token, QuillLogger& logger, Reactor& reactor)
        : ApplicationThread(token, logger, reactor, "DoubleSendConnectorThread", ThreadID{1}, make_queue_config(),
                            make_allocator_config("DoubleSendConnectorPool"), make_double_send_connector_thread_config()) {}
...
TEST_F(PduProtocolHandlerIntegrationTest, DoubleSendThenTeardown) {
    ...
    auto connector_thread = ApplicationThread::create<DoubleSendConnectorThread>(logger_->logger, *connector_reactor);
    connector_reactor->register_thread(connector_thread);
    ...
    EXPECT_TRUE(wait_for([&]() { return connector_thread->both_sent.load(std::memory_order_acquire); })) << "Connector: both PDUs not sent";
