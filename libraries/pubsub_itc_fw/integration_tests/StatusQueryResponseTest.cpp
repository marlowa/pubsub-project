...
TEST_F(StatusQueryResponseTest, StatusQueryResponseRoundTrip) {
    ...
    auto connector_thread = std::make_shared<ConnectorThread>(logger_->logger, *connector_reactor);
    ...
    EXPECT_TRUE(wait_for([&]() { return connector_thread->query_sent.load(std::memory_order_acquire); })) << "Connector: StatusQuery not sent";

    EXPECT_TRUE(wait_for([&]() { return listener_thread->query_received.load(std::memory_order_acquire); })) << "Listener: StatusQuery not received";

    EXPECT_TRUE(wait_for([&]() { return listener_thread->response_sent.load(std::memory_order_acquire); })) << "Listener: StatusResponse not sent";

    EXPECT_TRUE(wait_for([&]() { return connector_thread->response_received.load(std::memory_order_acquire); })) << "Connector: StatusResponse not received";
