...
FixParser::FixParser(pubsub_itc_fw::QuillLogger& logger, MessageCallback on_message)
...
bool FixParser::try_extract_message()
{
    ...
    if (!validate_checksum(msg_for_checksum, checksum_value)) {
        ...
        PUBSUB_LOG(logger_, pubsub_itc_fw::FwLogLevel::Debug,
            "FixParser: bad checksum -- computed {} received {} -- message discarded",
            computed, checksum_value);
        offset_ = soh3 + 1;
        return true;
    }
