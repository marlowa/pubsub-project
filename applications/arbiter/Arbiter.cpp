...
Arbiter::Arbiter(const ArbiterConfiguration& config, std::unique_ptr<pubsub_itc_fw::QuillLogger> logger) : config_(config), logger_(std::move(logger)) {
    ...
    reactor_configuration_.command_allocator_configuration_.handler_for_pool_exhausted = [this](void* /*ctx*/, int objects_per_pool) {
        PUBSUB_LOG(*logger_, pubsub_itc_fw::FwLogLevel::Warning,
                   "ArbiterCommandPool exhausted: chaining new pool slab ({} objects)", objects_per_pool);
    };
    ...
    PUBSUB_LOG((*logger_), pubsub_itc_fw::FwLogLevel::Info, "Arbiter: listening for sequencer connections on {}:{}", config_.listen_host, config_.listen_port);
}
