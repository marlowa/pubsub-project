...
ArbiterConfiguration ArbiterConfigurationLoader::load(const std::string& file_path) {
    ...
    try {
        ...
        if (config.event_queue_pool_objects_per_slab < 1) {
            throw pubsub_itc_fw::ConfigurationException("ArbiterConfigurationLoader: event_queue_pool.objects_per_slab must be >= 1, got " +
                                                        std::to_string(config.event_queue_pool_objects_per_slab));
        }
        if (config.event_queue_pool_initial_slabs < 1) {
            throw pubsub_itc_fw::ConfigurationException("ArbiterConfigurationLoader: event_queue_pool.initial_slabs must be >= 1, got " +
        ...
        if (config.command_queue_pool_objects_per_slab < 1) {
            throw pubsub_itc_fw::ConfigurationException("ArbiterConfigurationLoader: command_queue_pool.objects_per_slab must be >= 1, got " +
                                                        std::to_string(config.command_queue_pool_objects_per_slab));
        }
        if (config.command_queue_pool_initial_slabs < 1) {
            throw pubsub_itc_fw::ConfigurationException("ArbiterConfigurationLoader: command_queue_pool.initial_slabs must be >= 1, got " +
