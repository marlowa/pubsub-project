...
SampleFixGateway::SampleFixGateway(FixGatewayConfiguration  config) : config_(std::move(config)) {
    ...
    logger_ = std::make_unique<pubsub_itc_fw::QuillLogger>(log_file_name, pubsub_itc_fw::FileOpenMode{pubsub_itc_fw::FileOpenMode::Truncate},
    ...
    reactor_configuration_.command_allocator_configuration_.pool_name = "FixGatewayCommandPool";
    ...
    PUBSUB_LOG((*logger_), pubsub_itc_fw::FwLogLevel::Info, "SampleFixGateway: listening on {}:{}", config_.listen_host, config_.listen_port);
    PUBSUB_LOG((*logger_), pubsub_itc_fw::FwLogLevel::Info, "SampleFixGateway: SenderCompID={} TargetCompID={}", config_.sender_comp_id,
               config_.default_target_comp_id);
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (argc >= 2) {
            ...
            std::cout << "SampleFixGateway: loading configuration from " << config_file << "\n";
