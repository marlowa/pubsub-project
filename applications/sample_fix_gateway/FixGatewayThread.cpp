...
void FixGatewayThread::on_connection_established(pubsub_itc_fw::ConnectionID id)
{
    sessions_.emplace(
        ...
        std::forward_as_tuple(id, [this, id](const FixMessage& msg) {
            ...
            if (type == MsgType::Logon) {
                ...
                PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Warning,
                    "FixGatewayThread: connection {} received MsgType='{}' before Logon -- disconnecting",
                    id.get_value(), type);
                ...
                PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
                    "FixGatewayThread: connection {} ignoring MsgType='{}'",
                    id.get_value(), type);
            }
        }));

    // Start the logon timeout timer for this session.
    start_one_off_timer(sessions_.at(id).logon_timeout_timer_name(),
...
void FixGatewayThread::on_connection_lost(pubsub_itc_fw::ConnectionID id,
                                          const std::string& reason)
{
    ...
    PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
        "FixGatewayThread: connection {} lost: {} -- active sessions: {}",
...
void FixGatewayThread::on_raw_socket_message(const pubsub_itc_fw::EventMessage& message)
{
    ...
    if (it == sessions_.end()) {
        PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Warning,
            "FixGatewayThread::on_raw_socket_message: no session for connection {}",
            conn_id.get_value());
    ...
    if (!session.preamble_verified) {
        ...
        const std::size_t bytes_to_check = std::min(
            static_cast<std::size_t>(available),
            expected_preamble.size());

        if (std::memcmp(data, expected_preamble.data(), bytes_to_check) != 0) {
            PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Warning,
                "FixGatewayThread: connection {} invalid FIX preamble -- disconnecting",
                conn_id.get_value());
        ...
        PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
            "FixGatewayThread: connection {} FIX preamble verified",
            conn_id.get_value());
    }
    ...
    for (auto& [id, session] : sessions_) {
        if (session.logon_timeout_timer_name() == name) {
            if (!session.session_established) {
                PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Warning,
                    "FixGatewayThread: connection {} logon timeout -- disconnecting",
                    id.get_value());
...
void FixGatewayThread::handle_logon(FixSession& session, const FixMessage& msg)
{
    ...
    PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
        "FixGatewayThread: connection {} Logon from SenderCompID='{}'",
    ...
    PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
        "FixGatewayThread: connection {} FIX session established",
        session.conn_id.get_value());
}

void FixGatewayThread::handle_heartbeat(FixSession& session,
                                        [[maybe_unused]] const FixMessage& msg)
{
    PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Debug,
        "FixGatewayThread: connection {} Heartbeat",
        session.conn_id.get_value());
...
{
    PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
        "FixGatewayThread: connection {} TestRequest",
        session.conn_id.get_value());
...
{
    PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
        "FixGatewayThread: connection {} Logout: {}",
...
void FixGatewayThread::handle_new_order_single(FixSession& session, const FixMessage& msg)
{
    ...
    PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
        "FixGatewayThread: connection {} sent ExecutionReport OrderID={} ExecID={}",
...
{
    PUBSUB_LOG(get_logger(), pubsub_itc_fw::FwLogLevel::Info,
        "FixGatewayThread: disconnecting connection {}: {}",
...
{
    return "ORD-" + std::to_string(session.conn_id.get_value()) +
           "-" + std::to_string(session.order_id_counter++);
}

std::string FixGatewayThread::generate_exec_id(FixSession& session)
{
    return "EXEC-" + std::to_string(session.conn_id.get_value()) +
           "-" + std::to_string(session.exec_id_counter++);
}
