...
FixGatewayConfiguration FixGatewayConfigurationLoader::load(const std::string& file_path) {
    ...
    if (!ok) {
        throw pubsub_itc_fw::ConfigurationException("FixGatewayConfigurationLoader: failed to load '" + file_path + "': " + err);
    }
    ...
    try {
        ...
        if (listen_port < 1 || listen_port > 65535) {
            throw pubsub_itc_fw::ConfigurationException("FixGatewayConfigurationLoader: network.listen_port must be in range [1, 65535], got " +
        ...
        if (raw_buffer_capacity <= 0) {
            throw pubsub_itc_fw::ConfigurationException("FixGatewayConfigurationLoader: network.raw_buffer_capacity must be positive, got " +
        ...
        if (config.event_queue_pool_objects_per_slab < 1) {
            throw pubsub_itc_fw::ConfigurationException("FixGatewayConfigurationLoader: event_queue_pool.objects_per_slab must be >= 1, got " +
                                                        std::to_string(config.event_queue_pool_objects_per_slab));
        }
        if (config.event_queue_pool_initial_slabs < 1) {
            throw pubsub_itc_fw::ConfigurationException("FixGatewayConfigurationLoader: event_queue_pool.initial_slabs must be >= 1, got " +
        ...
        if (config.command_queue_pool_objects_per_slab < 1) {
            throw pubsub_itc_fw::ConfigurationException("FixGatewayConfigurationLoader: command_queue_pool.objects_per_slab must be >= 1, got " +
                                                        std::to_string(config.command_queue_pool_objects_per_slab));
        }
        if (config.command_queue_pool_initial_slabs < 1) {
            throw pubsub_itc_fw::ConfigurationException("FixGatewayConfigurationLoader: command_queue_pool.initial_slabs must be >= 1, got " +
