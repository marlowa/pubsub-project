...
// order gateway's open-order pool entry, so both are plain-old-data with no per-lookup
// heap allocation. Because it bounds those keys, every ClOrdID must be measured against
// it at the point it enters the system -- the order gateway (FIX ingress) -- and a
