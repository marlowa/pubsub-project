...
MatchingEnginePublisher::MatchingEnginePublisher(MatchingEnginePublisherConfiguration config, std::unique_ptr<pubsub_itc_fw::QuillLogger> logger)
    ...
    reactor_configuration_.command_allocator_configuration_.handler_for_pool_exhausted = [this](void*, int objects_per_pool) {
        PUBSUB_LOG(*logger_, pubsub_itc_fw::FwLogLevel::Warning, "MepCommandPool exhausted: chaining new pool slab ({} objects)", objects_per_pool);
    };
    ...
    reactor_->register_inbound_listener(pubsub_itc_fw::NetworkEndpointConfiguration{config_.listen_host, config_.orders_listen_port},
    ...
    reactor_->register_inbound_listener(pubsub_itc_fw::NetworkEndpointConfiguration{config_.listen_host, config_.er_listen_port}, pubsub_itc_fw::ThreadID{1},
    ...
    thread_ = pubsub_itc_fw::ApplicationThread::create<MatchingEnginePublisherThread>(*logger_, *reactor_, config_);
    reactor_->register_thread(thread_);

    // Outbound: sequencer WAL subscriber listeners.
    service_registry_.add("sequencer", pubsub_itc_fw::NetworkEndpointConfiguration{config_.sequencer_wal_host, config_.sequencer_wal_port},
                          pubsub_itc_fw::NetworkEndpointConfiguration{});
    service_registry_.add("sequencer_secondary",
                          pubsub_itc_fw::NetworkEndpointConfiguration{config_.sequencer_wal_secondary_host, config_.sequencer_wal_secondary_port},
                          pubsub_itc_fw::NetworkEndpointConfiguration{});
    ...
    PUBSUB_LOG(*logger_, pubsub_itc_fw::FwLogLevel::Info,
               "MatchingEnginePublisher: orders={}:{} er={}:{} sequencer_wal={}:{} sequencer_wal_secondary={}:{} instance_id={}", config_.listen_host,
               config_.orders_listen_port, config_.listen_host, config_.er_listen_port, config_.sequencer_wal_host, config_.sequencer_wal_port,
               config_.sequencer_wal_secondary_host, config_.sequencer_wal_secondary_port, config_.instance_id);
}
...
int main(int argc, char* argv[]) {
    if (pubsub_itc_fw::answer_hot_path_thread_count_query(argc, argv, matching_engine_publisher::MatchingEnginePublisher::hot_path_thread_count)) {
        return 0;
    }
    ...
    try {
        ...
        PUBSUB_LOG(*logger, pubsub_itc_fw::FwLogLevel::Error, "MatchingEnginePublisher: configuration error: {}", ex.what());
        return 1;
    }
