...
TEST_F(BinaryProtocolTest, EnvelopeCarriesTheOriginatingGatewayId) {
    ...
    EXPECT_EQ(view.origin_gateway_id, gateway_ids::binary_order_gateway);
...
TEST_F(BinaryProtocolTest, EnvelopeWithoutAGatewayIdStillDecodesAndMeansTheOrderGateway) {
    ...
    EXPECT_EQ(gateway_ids::default_when_absent, gateway_ids::fix_order_gateway);
}
...
TEST_F(BinaryProtocolTest, GatewayIdsAreDistinctAndStable) {
    EXPECT_EQ(gateway_ids::fix_order_gateway, 1);
    EXPECT_EQ(gateway_ids::binary_order_gateway, 2);
    EXPECT_NE(gateway_ids::fix_order_gateway, gateway_ids::binary_order_gateway);
}
...
TEST_F(BinaryProtocolTest, EnvelopeCarriesTheOriginatingGatewayInstance) {
    ...
    EXPECT_EQ(view.origin_gateway_id, gateway_ids::fix_order_gateway);
...
TEST_F(BinaryProtocolTest, InstanceIdsAreScopedToTheirProtocol) {
    ...
    EXPECT_NE(session_key(gateway_ids::fix_order_gateway, 1, 7), session_key(gateway_ids::binary_order_gateway, 1, 7));
    EXPECT_NE(session_key(gateway_ids::fix_order_gateway, 1, 7), session_key(gateway_ids::fix_order_gateway, 2, 7));
    EXPECT_EQ(session_key(gateway_ids::fix_order_gateway, 1, 7), session_key(gateway_ids::fix_order_gateway, 1, 7));
}
