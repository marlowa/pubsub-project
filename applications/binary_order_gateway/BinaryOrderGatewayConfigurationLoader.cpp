...
BinaryOrderGatewayConfigurationLoader::load_and_init_logging(const std::string& file_path, const std::string& log_file_path) {
    pubsub_itc_fw::TomlConfiguration toml;

    auto [ok, error] = toml.load_file(file_path);
    if (!ok) {
        throw pubsub_itc_fw::ConfigurationException("BinaryOrderGatewayConfigurationLoader: failed to load '" + file_path + "': " + error);
    }

    BinaryOrderGatewayConfiguration config;
    ...
    auto validate_port = [](int32_t port, const std::string& name) {
        if (port < 1 || port > 65535) {
            throw pubsub_itc_fw::ConfigurationException("BinaryOrderGatewayConfigurationLoader: " + name + " must be in range [1, 65535], got " +
                                                        std::to_string(port));
        }
    };
    auto validate_positive = [](int32_t value, const std::string& name) {
        if (value < 1) {
            throw pubsub_itc_fw::ConfigurationException("BinaryOrderGatewayConfigurationLoader: " + name + " must be >= 1, got " + std::to_string(value));
        }
    };
    ...
    if (instance_id < 1 || instance_id > INT16_MAX) {
        throw pubsub_itc_fw::ConfigurationException("BinaryOrderGatewayConfigurationLoader: gateway.instance_id must be 1 or greater "
    ...
    if (!pubsub_itc_fw::FwLogLevel::from_string(applog_level_text, config.applog_level)) {
        throw pubsub_itc_fw::ConfigurationException("BinaryOrderGatewayConfigurationLoader: logging.applog_level '" + applog_level_text +
                                                    "' is not a recognised log level");
    }
    if (!pubsub_itc_fw::FwLogLevel::from_string(syslog_level_text, config.syslog_level)) {
        throw pubsub_itc_fw::ConfigurationException("BinaryOrderGatewayConfigurationLoader: logging.syslog_level '" + syslog_level_text +
                                                    "' is not a recognised log level");
    }
    ...
    // Deliberately OUTSIDE the cpu_pinning_enabled block above. It was inside, which meant
    // a component with pinning turned off silently exposed no metrics at all -- no error,
    // no warning, just an endpoint that never appeared. Metrics and CPU pinning are
    // unrelated concerns, and the test harnesses now read their ground truth from these
    // counters, so a silent disable would make them pass while verifying nothing.
