...
#include "BinaryOrderGateway.hpp"
#include "BinaryOrderGatewayConfigurationLoader.hpp"
...
BinaryOrderGateway::BinaryOrderGateway(const BinaryOrderGatewayConfiguration& config, std::unique_ptr<pubsub_itc_fw::QuillLogger> logger)
    ...
    reactor_configuration_.command_allocator_configuration_.handler_for_pool_exhausted = [this](void* /*context*/, int objects_per_pool) {
        PUBSUB_LOG(*logger_, pubsub_itc_fw::FwLogLevel::Warning, "BinaryOrderGatewayCommandPool exhausted: chaining new pool slab ({} objects)",
                   objects_per_pool);
    };
    ...
    gateway_thread_ = pubsub_itc_fw::ApplicationThread::create<BinaryOrderGatewayThread>(*logger_, *reactor_, config_);
    reactor_->register_thread(gateway_thread_);

    // Outbound connections are initiated from BinaryOrderGatewayThread::on_app_ready_event()
    ...
    PUBSUB_LOG((*logger_), pubsub_itc_fw::FwLogLevel::Info, "BinaryOrderGateway: client listener on {}:{}", config_.listen_host, config_.listen_port);
    PUBSUB_LOG((*logger_), pubsub_itc_fw::FwLogLevel::Info, "BinaryOrderGateway: ER listener on {}:{}", config_.er_listen_host, config_.er_listen_port);
    PUBSUB_LOG((*logger_), pubsub_itc_fw::FwLogLevel::Info, "BinaryOrderGateway: authentication service {}:{} (SCRAM-SHA-256)",
               config_.authentication_service_host, config_.authentication_service_port);
    if (config_.ha_enabled) {
        PUBSUB_LOG((*logger_), pubsub_itc_fw::FwLogLevel::Info, "BinaryOrderGateway: sequencer primary={}:{} secondary={}:{} (HA enabled)",
        ...
        PUBSUB_LOG((*logger_), pubsub_itc_fw::FwLogLevel::Info, "BinaryOrderGateway: sequencer primary={}:{} (HA disabled)", config_.sequencer_primary_host,
                   config_.sequencer_primary_port);
    }
}

int BinaryOrderGateway::run() {
    PUBSUB_LOG_STR((*logger_), pubsub_itc_fw::FwLogLevel::Info, "BinaryOrderGateway: starting reactor");
...
int main(int argc, char* argv[]) {
    if (pubsub_itc_fw::answer_hot_path_thread_count_query(argc, argv, binary_order_gateway::BinaryOrderGateway::hot_path_thread_count)) {
        return 0;
    }

    if (argc != 3) {
        std::cerr << "Usage: binary_order_gateway <logfile> <config.toml>\n";
        return 1;
    }
    ...
    if (!writable_error.empty()) {
        std::cerr << "BinaryOrderGateway: " << writable_error << "\n";
        return 1;
    }

    pubsub_itc_fw::QuillLogger::block_signals_before_construction();

    binary_order_gateway::BinaryOrderGatewayConfiguration config;
    std::unique_ptr<pubsub_itc_fw::QuillLogger> logger;

    try {
        auto [loaded_config, initialised_logger] = binary_order_gateway::BinaryOrderGatewayConfigurationLoader::load_and_init_logging(config_file, log_file);
        config = std::move(loaded_config);
        logger = std::move(initialised_logger);
    } catch (const pubsub_itc_fw::ConfigurationException& ex) {
        std::cerr << fmt::format("BinaryOrderGateway: configuration error: {}\n", ex.what());
        return 1;
    }
    ...
    pubsub_itc_fw::ApplicationAnnouncer::announce(*logger, "binary_order_gateway");

    try {
        binary_order_gateway::BinaryOrderGateway app{config, std::move(logger)};
        return app.run();
    } catch (const std::exception& ex) {
        std::cerr << "BinaryOrderGateway: fatal exception: " << ex.what() << "\n";
        return 1;
    } catch (...) {
        std::cerr << "BinaryOrderGateway: unknown fatal exception\n";
        return 1;
    }
}
