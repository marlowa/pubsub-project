...
19. **Matching Engine HA wiring.** The cancel-on-failover design is agreed and documented in `docs/design/wal_and_ha.md` and `docs/applications/matching_engine.md`, but no code has been written. The ME currently runs as a single instance with no failover. Implementation requires: (a) ME-secondary tailing ME-primary's book-update replication channel; (b) arbiter-mediated promotion of ME-secondary on ME-primary failure; (c) ME-secondary reconciling its replicated book against the sequencer's WAL before issuing cancel ERs for genuinely-outstanding orders (the correctness rule — see `docs/design/wal_and_ha.md` "ME Failover Correctness Rule"). Depends on sequencer HA slice 8 (arbiter PSA+witness) landing first, since ME promotion requires an arbiter decision. Halt-on-failure remains the fallback for irreconcilable failure modes.
