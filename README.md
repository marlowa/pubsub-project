...
Self-signed certificates for a local sandbox are generated by the deploy flow (`--skip-certs` to reuse existing ones). See [docs/design/secure_comms.md](docs/design/secure_comms.md) for the design (why memory BIOs, the handshake state machine, and the SCRAM exchange).
